...
```{r}
ggplot(Q1_ans2018_subset, aes(x = reorder(Area, Salary),
                              y = Salary)) +
...
```{r}
ggplot(Q1_ans2019_subset, aes(x = reorder(Area, Salary),
                              y = Salary)) +
  geom_col(aes(fill = Job_Title)) +
  coord_flip() +
...
```{r}
ggplot(Q1_ans_subset, aes(x = reorder(Area, Salary),
                              y = Salary)) +
  geom_col(aes(fill = Job_Title)) +
  coord_flip() +
...
```{r}
ggplot(Q2_ans, aes(x = reorder(Sector, Salary),
                              y = Salary)) +
...
By looking at the figure above, it is proven that for 2018 and 2019, big companies in San Francisco always pay their employees higher than startups in New York City pay. Therefore, there is an advice for this case:
...
ggplot(SA_withoutNational, aes(x = Metro,
               y = Salary)) +
...
ggplot(JO_withoutNational, aes(x = Metro,
               y = Job_Openings)) +
  geom_boxplot(aes(fill = Metro), show.legend = F) +
