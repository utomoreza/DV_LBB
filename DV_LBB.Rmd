...
This article aims to meet the second LBB (Learn by Building) assignment from Algoritma, i.e. [Data Visualization](https://algorit.ma/course/data-visualization/) course. This article is based on [P4DS LBB](https://github.com/utomoreza/P4DS_LBB) question22222222question020/00//0article. However, there are some improvements which are as follows:
...
7. Show distributions of sectors' salary and job openings!
8. Show distributions of salary and job openings from Time Series!
...
plot4b <- ggplot(JO_withoutNational, aes(x = Metro,
               y = Job_Openings)) +
  geom_boxplot(aes(fill = Metro), show.legend = F) +
  geom_jitter(aes(color = as.factor(year(Date))), alpha = 0.3) +
  coord_flip() +
...
```{r}
Q5_DF1 <- QuickFacts_MetroMedianPay %>% mutate(Month = month(Date, label = T),
                                     Year = year(Date)) %>% 
  group_by(Area, Month, Year) %>% 
  summarise(Mean_MetroMedianPay = mean(Metro_Median_Pay))
Q5_DF2 <- QuickFacts_USMedianPay %>% mutate(Month = month(Date, label = T),
                                     Year = year(Date)) %>% 
  group_by(Area, Month, Year) %>% 
  summarise(Mean_USMedianPay = mean(US_Median_Pay))
```

## Show distributions of company size's salary and job openings!



## Show distributions of sectors' salary and job openings!



## Show distributions of salary and job openings from Time Series!
