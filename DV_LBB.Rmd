...
plot5 <- ggplot(Q5_DF_long, aes(x = reorder(Area, Value),
  ...
  scale_fill_brewer(palette = "Set2") +
  scale_y_continuous(labels = comma) +
  # scale_fill_manual(labels = c("Glassdoor Median Pay", "BLS Median Pay"), values = c("green", "red")) +
  theme_get() +
...
```{r}
options(scipen = 999)
ggplot(JobOpening_CompanySize, aes(x = Company_Size,
                                   y = Openings)) +
  geom_jitter(aes(color = Area)) +
  coord_flip() +
  scale_y_continuous(labels = comma) +
...
```{r}
ggplot(Salary_CompanySize, aes(x = Company_Size,
                                   y = Salary)) +
